...
class SetUpGameViewModel: ObservableObject, GameStateObserver {
    ...
    func incrementDoctor() {
        if doctorCount < 2 && doctorCount + detectiveCount + mafiaCount < players.count {
            doctorCount += 1
        }
    }
    ...
    func incrementDetective() {
        if detectiveCount < 2 && doctorCount + detectiveCount + mafiaCount < players.count {
            detectiveCount += 1
        }
    }
